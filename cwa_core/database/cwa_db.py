...
class CWA_DB:
    def __init__(self, verbose=False, db_path=None):
        ...
        if db_path:
            self.db_path = str(Path(db_path).parent) + "/"
        elif CWA_DB_PATH:
            self.db_path = str(CWA_DB_PATH.parent) + "/"
        else:
            self.db_path = "./"
...
"""
    ...
    def make_tables(self) -> tuple[list[str], list[str]]:
        ...
        schema = []
        for line in self.cwa_schema.split('\n'):
            if line.strip() and line.strip() != "":
                schema.append(line)
        
        tables = self.cwa_schema.split(';')
        # Remove empty strings and add semicolons back
